...
const UNIT_KEY = "/middle-1/literal-expressions";
const questions = literalExpressionsContent.diagnosticQuiz;
    ...
    <div className="grid gap-6">
      <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-[0_18px_60px_-34px_rgba(15,23,42,0.28)]">
        ...
        <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
          문자를 이름표처럼 받아들이는 감각과 식 읽기 흐름이 어느 정도 잡혀 있는지
          가볍게 확인해 보자.
        </p>
      </section>
        ...
        {!submitted ? (
          ...
          <div className="space-y-4">
            ...
            <p className="max-w-2xl text-base leading-7 text-slate-600">
              지금 점수를 출발점으로 삼고 AI 튜터와 같이 해 보면, 문자와 식을 숫자 이야기와
              연결하는 감각을 더 자연스럽게 만들 수 있어.
            </p>
            <div className="rounded-[1.5rem] bg-violet-50 p-5 text-sm leading-6 text-slate-700">
              <p className="font-bold text-slate-900">다음에 이어갈 새 문제 예시</p>
              <ul className="mt-3 space-y-2">
                {literalExpressionsContent.generatedProblems.slice(0, 2).map((problem) => (
