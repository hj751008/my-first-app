    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#fffdf7_0%,#eef7ff_45%,#f8fbff_100%)] px-6 py-10 text-slate-900">
      <section className="mx-auto flex w-full max-w-6xl flex-col gap-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <article className="rounded-[2rem] border border-sky-200 bg-white/90 p-8 shadow-[0_18px_60px_-34px_rgba(15,23,42,0.35)]">
            <p className="text-sm font-semibold uppercase tracking-[0.25em] text-sky-700">
              2단원 · {integersRationalContent.overview.title}
            </p>
            <h1 className="mt-4 text-3xl font-black sm:text-5xl">
              {integersRationalContent.overview.subtitle}
            </h1>
            <p className="mt-5 max-w-2xl text-base leading-7 text-slate-600">
              {integersRationalContent.overview.description}
            </p>

            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {integersRationalContent.overview.checkpoints.map((checkpoint) => (
                <div
                  key={checkpoint}
                  className="rounded-[1.5rem] bg-sky-50 p-4 text-sm leading-6 text-slate-700"
                >
                  {checkpoint}
                </div>
              ))}
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href="/middle-1/integers-and-rational-numbers/tutor"
                className="rounded-full bg-slate-950 px-6 py-3 text-sm font-bold text-white transition hover:bg-slate-800"
              >
                AI 튜터 시작하기
              </Link>
              <Link
                href="/middle-1/integers-and-rational-numbers/quiz"
                className="rounded-full border border-slate-300 px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-sky-400 hover:text-slate-900"
              >
                진단 퀴즈 먼저 해보기
              </Link>
            </div>
          ...
          <aside className="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-[0_18px_60px_-34px_rgba(15,23,42,0.28)]">
            ...
            <ul className="mt-5 space-y-4 text-sm leading-6 text-slate-600">
              {integersRationalContent.misconceptions.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
